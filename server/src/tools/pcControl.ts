...
import { requestOpenWindows, requestGizmos } from "./gameIpc";
 ...
 */
function foregroundGameWindow(imageName = "RimWorldWin64"): string {
...
`;
  const scriptPath = path.join(os.tmpdir(), `rwdt_focus_${randomBytes(6).toString("hex")}.ps1`);
  try {
    fs.writeFileSync(scriptPath, script, "utf8");
    const out = execSync(`powershell -NoProfile -ExecutionPolicy Bypass -File "${scriptPath}"`, { encoding: "utf8", timeout: 8000 });
...
function describeFocus(token: string): string {
  if (token === "focused") return "(RimWorld foregrounded)";
  if (token === "notfound") return "(focusGame requested but no RimWorld window found — is the game running?)";
...
async function captureAndCrop(
  ...
  uiW: number,
  uiH: number,
  opts: { pad?: number; format?: CaptureFormat; quality?: number; outName?: string }
): Promise<{ outPath: string; left: number; top: number; cropW: number; cropH: number; fullPath: string; focusToken: string }> {
  const focusToken = foregroundGameWindow();
  const fullPath = await captureScreen(opts.format, opts.quality);
  ...
  const ext = opts.format === CaptureFormat.JPEG ? "jpeg" : "png";
  const outPath = path.resolve(`${opts.outName || "window_crop"}.${ext}`);
  let pipeline = sharp()(fullPath).extract({ left, top, width: cropW, height: cropH });
  if (opts.format === CaptureFormat.JPEG) {
  ...
  try {
    switch (name) {
      case "capture_screen": {
        ...
        const filePath = await captureScreen(format, quality);
      ...
      case "capture_game_window": {
        ...
        const wins = await requestOpenWindows();
        if (!wins) {
      ...
      case "capture_region": {
        ...
        const filePath = await captureRegion(region, format, quality);
      ...
      case "launch_quicktest": {
        ...
        if (args.bare === true) {
          ...
          const watchdogNote = armGameWatchdog({ pid: child.pid ?? null, idleTimeoutMin: args.idleTimeoutMin });
        ...
        return await handleRimworldDevTool("launch_rimworld", { ...args, quicktest: true });
      }
      default:
